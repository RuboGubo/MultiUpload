...
def get_access_token(refresh_token):
    ...
    r = json.loads(requests.post("https://api.dailymotion.com/oauth/token",
                   data=access_token_data).text)  # requesting access & refresh token
    f = open(r'SignIn/UserPreferances.json', 'r')
    data = json.load(f)
    f.close()
    # saving refresh token in UserPreferances.json
...
def create_video(headers, url):
    VideoIdData = json.loads(requests.post(
        'https://api.dailymotion.com/me/videos', headers=headers, data={'url': url}).text)
    print(VideoIdData)
    if 'error' in VideoIdData:
        print("Error {}, {} Video not uploaded".format(
            VideoIdData["error"]["code"], VideoIdData["error"]["message"]))
    else:
        return VideoIdData["id"]


def publish_video(headers, VideoID):
    x = requests.post('https://api.dailymotion.com/video/{}?published=true&title={}&channel={}&tags={}&is_created_for_kids={}'.format(
        VideoID, title, channel, tags, is_created_for_kids), headers=headers)  # publishing video,
